    ...
    IChatManagerGAgent
{
    ...
    private async Task<UserProfileGAgent> GetUserProfileAgentAsync(Guid userId)
    {
        var agent = _agentFactory.CreateGAgent<UserProfileGAgent>(userId);
        await agent.ActivateAsync();
        return agent;
    }
        ...
        string? guider = null, DateTime? userLocalTime = null)
    {
        ...
        if (userProfile != null)
        {
            ...
            // Call UserProfileGAgent to set user information
            var userProfileAgent = await GetUserProfileAgentAsync(Id);
            await userProfileAgent.SetUserProfileAsync(
                userProfile.Gender,
