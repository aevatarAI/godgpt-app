...
test_record_app_rating() {
    ...
    local response=$(curl -k -s -X POST "$API_URL/api/godgpt/user-statistics/app-rating" \
...
test_can_user_rate_app() {
    ...
    local response=$(curl -k -s -X GET "$API_URL/api/godgpt/user-statistics/can-rate?deviceId=test_device_12345" \
        -H "Authorization: Bearer $ACCESS_TOKEN" \
        -H "Content-Type: application/json")
